...
def hyperp_search(classifier, parameters, dataframe, verbose=False):
    X = dataframe.drop(['likes'], axis=1)
    y = dataframe['likes']
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=313)

    gs = GridSearchCV(classifier, parameters, cv=5, verbose=1, n_jobs=8, scoring='neg_mean_absolute_error')
    ...
    if verbose is True:
        ...
        plt.hlines(y=0, xmin=0, xmax=20000, color="r")
        plt.xlim([0, 20000])
        plt.show()
...
def test_model(dataframe, scaler):
    model, params = get_RandomForestRegressor()
